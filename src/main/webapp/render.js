...
function renderXml(xml) {
    ...
    var xml_escaped = xmlformatted.replace(/&/g, '&amp;').replace(/</g, '&lt;')
            .replace(/>/g, '&gt;').replace(/ /g, '&nbsp;').replace(/\n/g,
                    '<br />');
    var prettified = '<code class="prettyprint">' + xml_escaped + '</code>';
    $('#ajaxformgetAll').append("<table id='countryList'></table>");

    $('#countryList').append('<tr><td>' + prettified + '</td></tr>');
}

function renderSingleXml(xml) {
    var xmlformatted = formatXml(xml);
    var endheader = xmlformatted.indexOf("?>", 0);
    var xmlwithoutheader = xmlformatted.substring(endheader + 2,
            xmlformatted.length);
    var xml_escaped = xmlwithoutheader.replace(/&/g, '&amp;').replace(/</g,
            '&lt;').replace(/>/g, '&gt;').replace(/ /g, '&nbsp;').replace(
            /\n/g, '<br />');
    // remove xml header

    var prettified = '<code class="prettyprint">' + xml_escaped + '</code>';
    $('#capitalRetrieved').html(prettified);
}
...
function renderList(data) {
    // JAX-RS serializes an empty list as null, and a 'collection of
    // one' as an object (not an 'array of one')
    data = data.countries;
    var list = data == null ? [] : (data instanceof Array ? data : [ data ]);
    $('#ajaxformgetAll').append("<table id='countryList'></table>");
    $.each(list, function(index, country) {
        $('#countryList').append(
                '<tr><td>' + country.name + '</td><td>' + country.capital
                        + '</td></tr>');
    });
}